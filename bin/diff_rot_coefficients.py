...
def f_1C_D0P(l, m, offdiag) : 
    ...
    if offdiag == -3 : 
        ...
        if l>=4 and l>=1+m and l+m>=1: 
            c = (1/5)*(1 + l)*(1 + 2*l)*np.sqrt(l/(-1 + 4*l**2))*((1 + l - 2*l**2)*np.sqrt(-((l**2 - m**2)/(l - 4*l**3))) + ((15*m**2*np.sqrt(-((l**2 - m**2)/(l - 4*l**3))))/(1 + l)) + 3*(27 - 6*l + 3*l**2 + l**3)*(-1 + l**2 - 5*m**2)*(np.sqrt((l**2 - m**2)/(l*(-1 + 4*l**2)))/np.abs(9 + 9*l - 4*l**2 - 4*l**3)))

        elif l == 3 and m>=-2 and m<=2: 
            c = -(4/5) * m**2 * np.sqrt(9 - m**2)
        elif l == 2 and m>=-1 and m<=1: 
...
def f_1C_lho1_D0P(l, m, offdiag) : 
    ...
    if offdiag == -3 : 
        ...
        c = (1/20)*(l+1)*np.sqrt(l*(l+2))*(2*l+1)*(30*(l+1)**1.5*(l*(l+2)-5*m**2)/(np.abs(1-2*l)*(2*l+1)*(2*l+3)*(2*l+5)*np.sqrt((l*(l+1)*(l+2))/((l-m+1)*(l+m+1))))+60*m**2/((l+1)*(2*l+1)*(2*l+3)*np.sqrt((l*(l+2))/((l-m+1)*(l+m+1))))-54*(l*(l+2)-5)*(l*(l+2)-5*m**2)/(np.abs(1-2*l)*(l+1)*(2*l+1)*(2*l+3)*(2*l+5)*np.sqrt((l*(l+2))/((l-m+1)*(l+m+1))))-4*np.sqrt(l*(l+2)*(l-m+1)*(l+m+1))/((l+1)*(2*l+1)*(2*l+3)))
    elif offdiag == 3 : 
        c = -(6 * l * (4 + l) * np.sqrt((1 + l - m) * (2 + l - m) * (3 + l - m) * (1 + l + m) * (2 + l + m) * (3 + l + m))) / ((3 + 2*l) * (5 + 2*l) * (7 + 2*l))
    return c

def f_1C_D1P(l, m, offdiag) : 
    """
    1-Curl equation : 
...
def f0_1C_lho1_D0P(l, m, offdiag) : 
    """
    1-Curl equation : 
    Coefficients for the [lho' x P_(l+Δl,m)] terms multiplied by the radial profile [f(r)].
    """
...
def f_2C_D0P(l, m, offdiag) : 
    ...
    if offdiag == -2 : 
        ...
        if l > 1 : 
            sl = np.sqrt(l*(l+1))
            c = -1j*np.sqrt(l*(l+1))*m*(-8*(sl*l**9) - 36*(sl*l**8) - 26*(sl*l**7) + 77*(sl*l**6) + 91*(sl*l**5) - 49*(sl*l**4) - 69*(sl*l**3) + 8*(sl*l**2) - 3*(-8*(sl*l**7) - 28*(sl*l**6) + 2*(sl*l**5) + 75*(sl*l**4) + 16*(sl*l**3) - 65*(sl*l**2) + 12*sl)*m**2 + 12*l*sl*(m**2 + 1)) / ((4*l**2 + 4*l - 3)**2 * (2*l**3 + 3*l**2 - 3*l - 2))
...
def f_2C_D1T(l, m, offdiag) : 
    ...
    if offdiag == -3 : 
        ...
        c4 = -(30*((l+1)**1.5)*(l**2+2*l-5*m**2)/(np.sqrt(l*(l**2+3*l+2)/(l**2+2*l-m**2+1))*np.abs(16*l**4+64*l**3+56*l**2-16*l-15)))
        c5 = - (54*(l**2+2*l-5)*(l**2+2*l-5*m**2)/(np.sqrt(l*(l+2)/(l**2+2*l-m**2+1))*np.abs(16*l**5+80*l**4+120*l**3+40*l**2-31*l-15)))
        c = c1*(c2+c3+c4+c5)
    elif offdiag == 3 : 
...
def df_2C_D0T(l, m, offdiag) : 
    ...
    if offdiag == -3 : 
        ...
        c = (1/20)*(l+1)*np.sqrt(l*(l+2))*(2*l+1)*((-4*np.sqrt(l*(l+2)*(l**2+2*l-m**2+1)))/(4*l**3+12*l**2+11*l+3) + 60*m**2/(np.sqrt(l*(l+2)/(l**2+2*l-m**2+1))*(4*l**3+12*l**2+11*l+3)) - 30*(l+1)**1.5*(l**2+2*l-5*m**2)/(np.sqrt(l*(l**2+3*l+2)/(l**2+2*l-m**2+1))*np.abs(16*l**4+64*l**3+56*l**2-16*l-15)) - 54*(l**2+2*l-5)*(l**2+2*l-5*m**2)/(np.sqrt(l*(l+2)/(l**2+2*l-m**2+1))*np.abs(16*l**5+80*l**4+120*l**3+40*l**2-31*l-15)))
    elif offdiag == 3 : 
        c = -(3*l*(l+4)*np.sqrt((l-m+1)*(l-m+2)*(l-m+3)*(l+m+1)*(l+m+2)*(l+m+3)))/((2*l+3)*(2*l+5)*(2*l+7))
    return c


def df_2C_lho1_D0P(l, m, offdiag) : 
    """
    2-Curl equation : 
...
def f0_2C_lho1_D0P(l, m, offdiag) : 
    """
    2-Curl equation : 
    Coefficients for the [lho' x P_(l+Δl,m)/r] terms multiplied by the radial profile [f(r)].
    """
...
def f0_2C_lho2_D0P(l, m, offdiag) : 
    """
    2-Curl equation : 
    Coefficients for the [lho'' x P_(l+Δl,m)] terms multiplied by the radial profile [f(r)].
    """
...
def f0_2C_lho1_D1P(l, m, offdiag) : 
    """
    2-Curl equation : 
    Coefficients for the [lho' x P'_(l+Δl,m)] terms multiplied by the radial profile [f(r)].
    """
...
def df0_2C_lho1_D0P(l, m, offdiag) : 
    """
    2-Curl equation : 
    Coefficients for the [lho' x P_(l+Δl,m)/r] terms multiplied by the radial profile [r f'(r)].
    """
...
# ------------------------------------------------------------------------------------------
# Coefficients for differential rotation interaction power : [(𝐮 ⋅ ∇Ω) r sin(θ) eϕ] . u
# ------------------------------------------------------------------------------------------
def w_SQ_20(l, m, offdiag) : 
    """
    Coefficients for the conj(S(l))*Q(l+offdiag) terms (Y20 part, coefficient multiplying svp(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    ...
    if offdiag == -2 : 
        if l>=3 and l>=2+m and l+m>=2: 
            c = -3j*(2*l+1)*np.sqrt((l**2 - 3*l + 2)/(16*l**3 - 24*l**2 - 4*l + 6))*m*np.sqrt(((l**2 - m**2)*(l**2 - 2*l - m**2 + 1))/(l*(2*l**2 - 7*l + 6)*(4*l**4 - 5*l**2 + 1)))
    elif offdiag == 0 : 
        if l <= 1 : 
            c = 1j*np.sqrt(2)*m/(5*np.sqrt(l*(l+1)))
        else : 
            c = 1j*np.sqrt(2)*m*np.sqrt(l*(l+1))*(-2*l**5 - 5*l**4 + 5*l**2 + (6*l**3 + 9*l**2 - 6)*m**2 + l*(2 - 9*m**2)) / (l*(8*l**6 + 28*l**5 + 14*l**4 - 35*l**3 - 28*l**2 + 7*l + 6))
    elif offdiag == 2 : 
        c = -3j*m*np.sqrt((l-m+1)*(l-m+2)*(l+m+1)*(l+m+2))/(np.sqrt(2)*np.sqrt(l*(l+1))*(2*l+3)*(2*l+5))
    return c

def w_SQ_00(l, m, offdiag) : 
    """
    Coefficients for the conj(S(l))*Q(l+offdiag) terms (Y00 part, coefficient multiplying spv(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    
    if offdiag == 0 : 
        c = -1j*np.sqrt(2)*m/np.sqrt(l*(1+l))
    return c

def w_TQ_20(l, m, offdiag) : 
    """
    Coefficients for the conj(T(l))*Q(l+offdiag) terms (Y20 part, coefficient multiplying svp(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    
    if offdiag == -3 : 
        if l>=3 and l>=3+m and l+m>=3: 
            c = 3*(2*l+1)*np.sqrt((l**3-2*l**2-l+2)/(16*l**4-64*l**3+56*l**2+16*l-15))*np.sqrt(((l-m-2)*(l-m-1)*(l+m-2)*(l+m-1)*(l**2-m**2))/((l-2)*l*(2*l-5)*(2*l-3)*(4*l**3-4*l**2-l+1)))/np.sqrt(2)
    elif offdiag == -1 : 
        if l>=2 and l>=1+m and l+m>=1: 
            c = -(2*l+1)*(2*np.sqrt((l+1)*(l**2-m**2)/(l*(1-4*l**2)**2)) - 3*(l+6)*np.sqrt((l-1)/(16*l**4-40*l**2+9))*(l**2-5*m**2-1)*np.sqrt(-(l**2-m**2)/(-16*l**7+56*l**5-49*l**3+9*l)))/(5*np.sqrt(2))
        elif l>=1+m and l+m>=1:
            c = np.sqrt(2)*np.sqrt((l+1)*(l**2-m**2)/l)/(5-10*l)
    elif offdiag == 1 : 
        c = (l**3 + 5*l**2 + l*(3*m**2 + 4) - 15*m**2)/(np.sqrt(2)*(2*l+3)*(4*l**2 + 8*l - 5)*np.sqrt(l*(l+1)/(l**2 + 2*l - m**2 + 1)))
    elif offdiag == 3 : 
        c = -3*np.sqrt(l*(l-m+1)*(l-m+2)*(l-m+3)*(l+m+1)*(l+m+2)*(l+m+3)/(l+1))/(np.sqrt(2)*(2*l+3)*(2*l+5)*(2*l+7))
    return c

def w_TQ_00(l, m, offdiag) : 
    """
    Coefficients for the conj(T(l))*Q(l+offdiag) terms (Y00 part, coefficient multiplying spv(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    ...
    if offdiag == -1 : 
        if l>=1+m and l+m>=1:
            c = np.sqrt(2)*np.sqrt((l+1)*(l**2-m**2)/l)/(2*l-1)
    elif offdiag == 1 : 
        c = -np.sqrt(2)*np.sqrt(l*(l**2 + 2*l - m**2 + 1)/(l+1))/(2*l + 3)
    return c

def w_SS_20(l, m, offdiag) : 
    """
    Coefficients for the conj(S(l))*S(l+offdiag) terms (Y20 part, coefficient multiplying aub(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    ...
    if offdiag == -2 : 
        if l>=3 and l>=2+m and l+m>=2: 
            c = 3j*np.sqrt(2)*(l-2)*(2*l+1)*np.sqrt(l**2-3*l+2)*np.sqrt(1/(8*l**3 - 12*l**2 - 2*l + 3))*m*np.sqrt(((l**2 - m**2)*(l**2 - 2*l - m**2 + 1))/(l*(2*l**2 - 7*l + 6)*(4*l**4 - 5*l**2 + 1)))
    elif offdiag == 0 : 
        if l < 1 : 
            c = 3j*np.sqrt(2)*m/(5*np.sqrt(l*(l+1)))
        else : 
            c = -3j*np.sqrt(2)*m*(l**2 + l - 3*m**2)/(np.sqrt(l*(l+1))*(4*l**2 + 4*l - 3))
    elif offdiag == 2 : 
        c = -3j*np.sqrt(2)*(l+3)*m*np.sqrt((l-m+1)*(l-m+2)*(l+m+1)*(l+m+2)/(l*(l+1)))/((2*l+3)*(2*l+5))
    return c

def w_TS_20(l, m, offdiag) : 
    """
    Coefficients for the conj(T(l))*S(l+offdiag) terms (Y20 part, coefficient multiplying aub(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    ...
    if offdiag == -3 : 
        if l>=4 and l>=3+m and l+m>=3: 
            c = -3*np.sqrt(2)*(2*l+1)*np.sqrt(l**2-5*l+6)*np.sqrt((l**3-3*l**2-l+3)/(16*l**4-64*l**3+56*l**2+16*l-15))*np.sqrt(((l-m-2)*(l-m-1)*(l+m-2)*(l+m-1)*(l**2-m**2))/((l-2)*l*(2*l-5)*(2*l-3)*(4*l**3-4*l**2-l+1)))
    elif offdiag == -1 : 
        if l>=2 and l>=1+m and l+m>=1: 
            c = 3*np.sqrt((l-1)*l)*(2*l+1)*( (2*(l**2+9)*np.sqrt(1/(16*l**4 - 40*l**2 + 9))*(l**2-5*m**2-1)*np.sqrt((l**2-m**2)/(16*l**6 - 56*l**4 + 49*l**2 - 9))/l) - (2*np.sqrt((l**2-1)*(l**2-m**2))/(l-4*l**3)) )/(5*np.sqrt(2))
    elif offdiag == 1 : 
        c = 3*(2*l+1)*np.sqrt(l**2+3*l+2)*( (4*np.sqrt(l*(l+2)*(l**2+2*l-m**2+1))/(4*l**3+12*l**2+11*l+3)) + (10*(l+1)**1.5*(l**2+2*l-5*m**2)/np.sqrt(l*(l**2+3*l+2)*(16*l**4+64*l**3+56*l**2-16*l-15)**2/(l**2+2*l-m**2+1))) - (6*(l**2+2*l-5)*(l**2+2*l-5*m**2)/np.sqrt(l*(l+2)*(16*l**5+80*l**4+120*l**3+40*l**2-31*l-15)**2/(l**2+2*l-m**2+1))) )/(10*np.sqrt(2))
    elif offdiag == 3 : 
        c = -3*np.sqrt(2)*(l+4)*np.sqrt(l*(l-m+1)*(l-m+2)*(l-m+3)*(l+m+1)*(l+m+2)*(l+m+3)/(l+1))/((2*l+3)*(2*l+5)*(2*l+7))
    return c

def w_ST_20(l, m, offdiag) : 
    """
    Coefficients for the conj(S(l))*T(l+offdiag) terms (Y20 part, coefficient multiplying aub(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    ...
    if offdiag == -1 : 
        if l>=2 and l>=1+m and l+m>=1:
            c = 3*np.sqrt((l-1)*l)*(2*l+1)*( (2*np.sqrt((l**2-1)*(l**2-m**2))/(l-4*l**3)) + (2*(4*l**2-9)*np.sqrt(1/(16*l**4-40*l**2+9))*(l**2-5*m**2-1)*np.sqrt((l**2-m**2)/(16*l**6-56*l**4+49*l**2-9))/l) )/(5*np.sqrt(2))
    elif offdiag == 1 : 
        c = 3*np.sqrt(2)*np.sqrt(l**2+3*l+2)*(-5*m**2*np.sqrt((l**2+2*l-m**2+1)/(l**2+2*l)) + 2*np.sqrt(l*(l**2+2*l-m**2+1)/(l+2)) - np.sqrt(l*(l+2)*(l**2+2*l-m**2+1)) + np.sqrt(l**3*(l**2+2*l-m**2+1)/(l+2)))/(5*(l+1)*(2*l+3))
    return c

def w_TT_20(l, m, offdiag) : 
    """
    Coefficients for the conj(T(l))*T(l+offdiag) terms (Y20 part, coefficient multiplying aub(r))
    """
    l = np.float64(l); m = np.float64(m)
    c = 0
    ...
    if offdiag == -2 : 
        if l>=3 and l>=2+m and l+m>=2: 
            c = -3j*np.sqrt(2)*(l+1)*(2*l+1)*np.sqrt(l**2-3*l+2)*np.sqrt(1/(8*l**3 - 12*l**2 - 2*l + 3))*m*np.sqrt(((l**2 - m**2)*(l**2 - 2*l - m**2 + 1))/(l*(2*l**2 - 7*l + 6)*(4*l**4 - 5*l**2 + 1)))
    elif offdiag == 0 : 
        if l <= 1 : 
            c = 3j*np.sqrt(2)*m/(5*np.sqrt(l*(l+1)))
        else : 
        ...
        c = 3j*np.sqrt(2)*l*m*np.sqrt((l-m+1)*(l-m+2)*(l+m+1)*(l+m+2)/(l*(l+1)))/((2*l+3)*(2*l+5))
    return c
