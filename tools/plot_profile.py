...
dint = np.real(np.sum(Dint_pol+Dint_tor,0)*par.Ek)
dohm = np.real(np.sum(Dohm_pol+Dohm_tor,0)*par.Le2*par.Em)
#rb = r[r>1-100*sqrt(Ek)]
#hb = hv[r>1-100*sqrt(Ek)]
#h0 = rb[hb==max(hb)]
#g0 = r[(dint>=0.01*dint[0])&(r>0.99)][-1]
