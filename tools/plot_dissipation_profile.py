...
plt.figure();
yint = dint/K[solnum];
plt.plot(r,yint, label=r'$\mathcal{D}_\mathrm{int}$')
...
if par.magnetic == 1:
	...
	dohm_pol = np.zeros((int((lmm+1)/2), nR),dtype=complex)
	dohm_tor = np.zeros((int((lmm+1)/2), nR),dtype=complex)

	Plj0 = reb[:n] + 1j*imb[:n] 		#  N elements on each l block
	Tlj0 = reb[n:n+n] + 1j*imb[n:n+n] 	#  N elements on each l block
	Plj   = np.reshape(Plj0,(-1,N))
	Tlj   = np.reshape(Tlj0,(-1,N))
	...
	# the b field has opposite symmetry to that of u (if imposed background field is antisymmetric)
	lbf = lut
	lbg = lup	

	Lbf = ss.diags(lbf*(lbf+1),0)
	Lbg = ss.diags(lbg*(lbg+1),0)
	Inv_Lbf = ss.diags(1/(lbf*(lbf+1)),0)
	Inv_Lbg = ss.diags(1/(lbg*(lbg+1)),0)
	...
	for k,l in enumerate(lbf):
		...
		# Ohmic dissipation: 
		f0 = 4*np.pi*L/(2*l+1)
		f1 = np.absolute( q0 - s0 - r*s1 )**2
		dohm_pol[k,:] = f0*( f1 )
	

	## Toroidal components
	for k,l in enumerate(lbg):
		...
		# Ohmic dissipation: 
		f0 = 4*np.pi*L/(2*l+1)
		f1 = np.absolute( r*t1 + t0 )**2
		f2 = L*np.absolute( t0 )**2 
		dohm_tor[k,:] = f0*( f1+f2 )
	...
	yohm = dohm/K[solnum];
	plt.plot(r,yohm, label=r'$\mathcal{D}_\mathrm{ohm}$')
...
k = np.argsort(r)
tmp = yint+yohm
print('Estimated damping:', -np.trapz(tmp[k],r[k]))
print('   Actual damping:', w[solnum][0])
