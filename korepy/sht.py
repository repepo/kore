	...
	def get_sol(self,datDir):
		...
		if par.magnetic == 1:
			...
			if self.m > 0 :
				...
				if bsymm == 1:
					m_top = self.m
					m_bot = self.m+1						# equatorially symmetric case (symm=1)
					lmax_top = self.lmax
					lmax_bot = self.lmax+1
				elif bsymm == -1:
					m_top = self.m+1
					m_bot = self.m				# equatorially antisymmetric case (symm=-1)
					lmax_top = self.lmax+1
					lmax_bot = self.lmax
			elif self.m == 0 :
				symm1 = -bsymm 
			...
			br     = np.zeros([ntheta,nphi,self.nr]) 
			btheta = np.zeros([ntheta,nphi,self.nr])
			bphi   = np.zeros([ntheta,nphi,self.nr])
			...
			for ir in range(self.nr):
				br[...,ir],btheta[...,ir],bphi[...,ir] = sh.synth(Q[:,ir], S[:,ir], T[:,ir])


			# Transpose to form (nphi, ntheta, self.nr) - old magic style habit

			br     = np.transpose(ur, (1,0,2))
			btheta = np.transpose(utheta, (1,0,2))
			bphi   = np.transpose(uphi, (1,0,2))
